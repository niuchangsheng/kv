...
private:
    // 使用 std::map 实现有序存储（当前实现）
    // 未来可考虑使用 SkipList 优化性能
    std::map<std::string, std::string> table_;
    size_t approximate_size_;
    static const size_t kMaxSize = 4 * 1024 * 1024;  // 4MB（后备值）
};
```
...
#### kMaxSize vs write_buffer_size

**设计关系**：
- `kMaxSize`：MemTable 类的**硬编码默认值**（4MB），作为后备值
- `write_buffer_size`：用户可配置的**运行时阈值**，通过 Options 传入

**实现逻辑**：
```cpp
// 在 DB::MaybeScheduleFlush() 中使用
size_t threshold = options_.write_buffer_size > 0 
    ? options_.write_buffer_size 
    : 4 * 1024 * 1024;  // 后备值（与 kMaxSize 相同）

if (memtable_->ApproximateSize() > threshold) {
    // 转换为 Immutable MemTable，准备刷新
}
```
...
- 如果 `write_buffer_size` 未设置，使用 4MB 常量作为后备值
...
**设计权衡**：
- ✅ **优点**：简单、高效、易于实现
- ⚠️ **缺点**：无法区分"空值"和"删除标记"（当前实现中空值是有效的，删除标记是单个 null 字节）
...
### 4.5 数据结构选择：std::map vs SkipList
