...
- [ ] 是否运行了长时间运行的数据正确性测试？(`./bin/kv_data_correctness`)
- [ ] 长时间测试是否没有发现数据损坏？
- [ ] 是否在关键变更后运行了长时间测试？
...
- **执行**: `./bin/kv_data_correctness`（独立测试程序）
- **配置**: 通过环境变量控制
...
- **示例**:
  ```bash
  # 运行30秒的长时间测试（默认）
  ./bin/kv_data_correctness
  
  # 运行3秒的快速测试
  KV_TEST_DURATION_SECONDS=3 ./bin/kv_data_correctness
  
  # 运行60秒的长时间测试，使用2000个键
  KV_TEST_DURATION_SECONDS=60 KV_TEST_NUM_KEYS=2000 ./bin/kv_data_correctness
  ```
- **验证内容**:
  - 持续写入和读取数据
  - 定期验证所有已写入的数据是否正确
